package com.adesk.pianokeyboardlib.keyboard;
...
import com.adesk.pianokeyboardlib.Const;
...
public class PianoKeyBoard extends View {
    ...
    private int pronuncTextType = 1;
    private int keyBoardDefaultStartPosition = 0;
    private int blackKeyContinue = -1;
    ...
    private void init(AttributeSet attrs, int defStyle) {
        final TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.PianoKeyBoard, defStyle, 0);

        pronuncTextColor = a.getColor(R.styleable.PianoKeyBoard_pronuncTextColor, pronuncTextColor);
        pronuncTextDimension = a.getDimension(R.styleable.PianoKeyBoard_pronuncTextSize, pronuncTextDimension);
        ...
        if (a.hasValue(R.styleable.PianoKeyBoard_whiteKeyDrawable)) {
            whiteKeyDrawable = (BitmapDrawable) a.getDrawable(R.styleable.PianoKeyBoard_whiteKeyDrawable);
        }
        if (a.hasValue(R.styleable.PianoKeyBoard_whiteKeyPressedDrawable)) {
            whiteKeyPressedDrawable = (BitmapDrawable) a.getDrawable(R.styleable.PianoKeyBoard_whiteKeyPressedDrawable);
        }
        if (a.hasValue(R.styleable.PianoKeyBoard_blackKeyDrawable)) {
            blackKeyDrawable = (BitmapDrawable) a.getDrawable(R.styleable.PianoKeyBoard_blackKeyDrawable);
        }
        if (a.hasValue(R.styleable.PianoKeyBoard_blackKeyPressedDrawable)) {
            blackKeyPressedDrawable = (BitmapDrawable) a.getDrawable(R.styleable.PianoKeyBoard_blackKeyPressedDrawable);
        }
        if (a.hasValue(R.styleable.PianoKeyBoard_pronuncTextType)) {
            pronuncTextType = a.getInt(R.styleable.PianoKeyBoard_pronuncTextType, pronuncTextType);
        }
        if (a.hasValue(R.styleable.PianoKeyBoard_keyBoardDefaultStartPosition)) {
            keyBoardDefaultStartPosition = a.getInt(R.styleable.PianoKeyBoard_keyBoardDefaultStartPosition, keyBoardDefaultStartPosition);
        }
        if (a.hasValue(R.styleable.PianoKeyBoard_blackKeyContinue)) {
            blackKeyContinue = a.getInt(R.styleable.PianoKeyBoard_blackKeyContinue, blackKeyContinue);
        }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        initKeys();
        moveToPosition(keyBoardDefaultStartPosition);
    }
    ...
    private void initKeys() {
        ...
        list.addAll(WhiteKey.generatorWhiteKey(whiteKeyWidth, keyBoardHeight, whiteKeyDrawable.getBitmap(), whiteKeyPressedDrawable.getBitmap(), pronuncTextPaint, whiteKeyHeight * pronuncTextYRatio, pronuncTextType));
        list.addAll(BlackKey.generatorBlackKey(whiteKeyWidth, blackKeyWidth, blackKeyHeight, blackKeyDrawable.getBitmap(), blackKeyPressedDrawable.getBitmap(), blackKeyContinue));
        reverseList.clear();
    ...
    /**
     * 设置白键上字体的样式, 参照Const定义
     *
     * @param pronuncTextType
     */
    public void setPronuncTextType(int pronuncTextType) {
        this.pronuncTextType = pronuncTextType;
        initKeys();
    }
     ...
     * 设置默认开始位置, 以白键为数
     *
     * @param keyBoardDefaultStartPosition
     */
    public void setKeyBoardDefaultStartPosition(int keyBoardDefaultStartPosition) {
        this.keyBoardDefaultStartPosition = keyBoardDefaultStartPosition;
        initKeys();
    }

    /**
     * 设置默认需要过滤不显示的某个黑键, 只能取Const.GAPS定义值, 默认值-1 不过滤
     *
     * @param blackKeyContinue
     */
    public void setBlackKeyContinue(int blackKeyContinue) {
        this.blackKeyContinue = blackKeyContinue;
        initKeys();
    }
