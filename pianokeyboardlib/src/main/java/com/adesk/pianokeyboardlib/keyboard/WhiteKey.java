...
public class WhiteKey extends Key {
    ...
    public static List<WhiteKey> generatorWhiteKey(float whiteKeyWidth, float whiteKeyHeight, Bitmap unPressedBitmap, Bitmap pressedBitmap, TextPaint whiteTextPaint, float textYCoordinate, int pronuncTextType) {
        ...
        for (int i = 0; i < Const.RANGE.length; i++) {
            ...
            if (pronuncTextType == 3) {
                whiteKey.setTextToDraw(Const.PRONUNCIATION_INIT_CAP[i]);
            } else if (pronuncTextType == 2) {
                whiteKey.setTextToDraw(Const.PRONUNCIATION[i]);
            } else {
